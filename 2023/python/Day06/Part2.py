...
def main():
    ...
    time = int(''.join(time.split(':')[1].strip().split()))
    dist = int(''.join(dist.split(':')[1].strip().split()))
    ...
    # start < t < stop
    start = math.ceil(0.5 * (time - (time**2 - 4 * dist)**0.5))
    stop = math.floor(0.5 * (time + (time**2 - 4 * dist)**0.5))
