...
void *threadTraverse(void *arg) {
	threadArg_t args = *(threadArg_t *)arg;
	grid_t grid = args.grid;
	pt_t pt = args.pt;
	int *newCount = args.newCount;
...
void *leftThread(void *arg) {
	...
	int counts[grid.height];
	threadArg_t args[grid.height];
	pthread_t ids[grid.height];

	for (size_t i = 0; i < grid.height; ++i) {
		args[i].grid = grid;
		args[i].pt = (pt_t){grid.width - 1, i, left};
		args[i].newCount = counts + i;
		pthread_create(ids + i, NULL, &threadTraverse, args + i);
	}

	for (size_t i = 0; i < grid.height; ++i) {
		pthread_join(ids[i], NULL);
		maxLeft = counts[i] > maxLeft ? counts[i] : maxLeft;
	}
...
void *rightThread(void *arg) {
	...
	for (size_t i = 0; i < grid.height; ++i) {
		pthread_join(ids[i], NULL);
		maxRight = counts[i] > maxRight ? counts[i] : maxRight;
	}
...
void *upThread(void *arg) {
	...
	for (size_t i = 0; i < grid.width; ++i) {
		args[i].grid = grid;
		args[i].pt = (pt_t){i, grid.height - 1, up};
		args[i].newCount = counts + i;
		pthread_create(ids + i, NULL, &threadTraverse, args + i);
	}
	...
	for (size_t i = 0; i < grid.height; ++i) {
		pthread_join(ids[i], NULL);
		maxUp = counts[i] > maxUp ? counts[i] : maxUp;
	}
...
void *downThread(void *arg) {
	...
	for (size_t i = 0; i < grid.height; ++i) {
		pthread_join(ids[i], NULL);
		maxDown = counts[i] > maxDown ? counts[i] : maxDown;
	}
