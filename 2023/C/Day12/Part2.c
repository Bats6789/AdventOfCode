...
void parseLine(const char *line, char **row, ull **coord, size_t *coordSz) {
    ...
    *row = malloc(sizeof(**row) * ((tmp - line + 1) * 5 + 4));
    strncpy(*row, line, (tmp - line));
	shift = *row + (tmp - line);
	for (int i = 0; i < 4; ++i) {
		*shift = '?';
		shift++;
		strncpy(shift, line, (tmp - line));
		shift += (tmp - line);
	}
	*shift = '\0';
    ...
    *coord = malloc(sizeof(**coord) * *coordSz);
    ...
    (*coord)[count++] = val;
	originalCount = count;
	*coordSz = originalCount * 5;
	*coord = realloc(*coord, sizeof(**coord) * *coordSz);

	for (count = originalCount; count < originalCount * 5; ++count) {
		(*coord)[count] = (*coord)[index++];
...
ull getScore(const char *row, size_t rowSz, const ull *coord, size_t coordSz,
    ...
    for (int i = 0; i < 2; ++i) {
        if (row[index] == checkMap[i] || row[index] == '?') {
            ...
            } else if (checkMap[i] == '#' && coord[coordIndex] > current) {
