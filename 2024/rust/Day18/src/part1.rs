use std::{
    cmp::Reverse, collections::HashSet, env, fs::File, io::{self, prelude::*, BufReader}
};
...
fn main() -> io::Result<()> {
    ...
    while !pq.is_empty() && loc != end {
        ...
        let tmp = Point {x: loc.x - 1, y: loc.y};
        if loc.x > 0 && grid[tmp.y as usize][tmp.x as usize] != '#' && !seen.contains(&tmp) {
            pq.push(tmp, Reverse(step + 1));
        }

        let tmp = Point {x: loc.x + 1, y: loc.y};
        if loc.x < (grid[0].len() as i64) - 1 && grid[tmp.y as usize][tmp.x as usize] != '#' && !seen.contains(&tmp) {
            pq.push(tmp, Reverse(step + 1));
        }

        let tmp = Point {x: loc.x, y: loc.y - 1};
        if loc.y > 0 && grid[tmp.y as usize][tmp.x as usize] != '#' && !seen.contains(&tmp) {
            pq.push(tmp, Reverse(step + 1));
        }

        let tmp = Point {x: loc.x, y: loc.y + 1};
        if loc.y < (grid.len() as i64) - 1 && grid[tmp.y as usize][tmp.x as usize] != '#' && !seen.contains(&tmp) {
            pq.push(tmp, Reverse(step + 1));
        }
    }
