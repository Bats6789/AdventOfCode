...
fn region(grid: &Vec<Vec<char>>, x: usize, y: usize, el: char, seen: &mut Vec<Vec<bool>>) -> Parameters {
    ...
    // outside top left
    if (x == 0 || grid[y][x - 1] != el) && (y == 0 || grid[y - 1][x] != el) {
        res.perm += 1;
    }
    // outside top right
    if (x == grid[0].len() - 1 || grid[y][x + 1] != el) && (y == 0 || grid[y - 1][x] != el) {
        res.perm += 1;
    }
    // outside bottom left
    if (x == 0 || grid[y][x - 1] != el) && (y == grid.len() - 1 || grid[y + 1][x] != el) {
        res.perm += 1;
    }
    // outside bottom right
    if (x == grid[0].len() - 1 || grid[y][x + 1] != el) && (y == grid.len() - 1 || grid[y + 1][x] != el) {
        res.perm += 1;
    }

    // inside top left
    if (x > 0 && grid[y][x - 1] == el) && (y > 0 && grid[y - 1][x] == el) && grid[y - 1][x - 1] != el {
        res.perm += 1;
    }
    // inside top right
    if (x < grid[0].len() - 1 && grid[y][x + 1] == el) && (y > 0 && grid[y - 1][x] == el) && grid[y - 1][x + 1] != el {
        res.perm += 1;
    }
    // inside bottom left
    if (x > 0 && grid[y][x - 1] == el) && (y < grid.len() - 1 && grid[y + 1][x] == el) && grid[y + 1][x - 1] != el {
        res.perm += 1;
    }
    // inside bottom right
    if (x < grid[0].len() - 1 && grid[y][x + 1] == el) && (y < grid.len() - 1 && grid[y + 1][x] == el) && grid[y + 1][x + 1] != el {
        res.perm += 1;
    }
