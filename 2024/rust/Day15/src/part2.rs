use std::{
    env, fs::File, io::{self, prelude::*, BufReader}, ops::{Add, AddAssign}
};
...
fn get_shifts(grid: &Vec<Vec<char>>, pos: Point, d: Point, shift_list: &mut Vec<Shift>) -> bool {
    if grid[pos.y as usize][pos.x as usize] == '.' {
        ...
        if get_shifts(grid, pos + d, d, shift_list) {
            let shift = Shift {
                to: pos + d,
                from: pos,
            };
        ...
        let x_shift = Point {x: 1, y: 0};

        if get_shifts(grid, pos + d, d, shift_list) &&
           get_shifts(grid, pos + d + x_shift, d, shift_list){
            let shift = Shift {
                to: pos + d,
                from: pos,
            };
...
fn main() -> io::Result<()> {
    ...
    let mut grid: Vec<Vec<char>> = parts
        ...
        .unwrap()
        .split_whitespace()
        .map(|l| l.chars().flat_map(|c| match c {
            'O' => vec!['[', ']'].into_iter(),
            '@' => vec!['@', '.'].into_iter(),
            _ => vec![c, c].into_iter()
        }).collect())
