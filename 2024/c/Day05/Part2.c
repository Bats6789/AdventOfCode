...
void reorder(array_t page, array_t order[100]);
...
size_t find_index(array_t array, uint64_t value) {
...
void reorder(array_t page, array_t order[100]) {
	...
	while (!isvalid(page, order)) {
		for (size_t i = 0; i < 100; ++i) {
			seen[i] = false;
		}
		swap = false;
		for (size_t i = 0; i < page.sz && !swap; ++i) {
			array_t o = order[page.data[i]];
			for (size_t j = 0; j < o.sz && !swap; ++j) {
				if (seen[o.data[j]]) {
					int tmp = page.data[i];
					size_t found_index = find_index(page, o.data[j]);
					page.data[i] = page.data[found_index];
					page.data[found_index] = tmp;
				}
			}
			seen[page.data[i]] = true;
		}
