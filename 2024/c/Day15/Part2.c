...
bool point_equal(point_t left, point_t right) { return left.x == right.x && left.y == right.y; }
...
bool get_shifts(grid_t grid, point_t pos, point_t d, array_t *shift_list) {
    ...
    if (grid.grid[pos.y * grid.w + pos.x] == '.') {
        ...
        if (get_shifts(grid, new_point, d, shift_list)) {
        ...
        point_t new_point2 = {pos.x + d.x + 1, pos.y + d.y};
        if (get_shifts(grid, new_point, d, shift_list) && get_shifts(grid, new_point2, d, shift_list)) {
            shift_t shift = {new_point, pos};
            if (!in_array(*shift_list, shift)) {
                append_array(shift_list, shift);
            }
