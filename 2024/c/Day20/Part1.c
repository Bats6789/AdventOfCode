...
int main(int argc, char **argv) {
	...
	for (point_t walk = {0, 0}; walk.y < grid.h; ++walk.y) {
		for (walk.x = 0; walk.x < grid.w; ++walk.x) {
			if (grid.grid[walk.y * grid.w + walk.x] == '#') {
				continue;
			}
			...
			to.x -= 2;
			size_t from_i = walk.y * grid.w + walk.x;
			size_t to_i = to.y * grid.w + to.x;
			if (to.x > 0 && grid.grid[to_i] != '#' && dist[to_i] - dist[from_i] >= 102) {
				count += 1;
			}
			...
			if (to.x < grid.w - 1 && grid.grid[to_i] != '#' && dist[to_i] - dist[from_i] >= 102) {
				count += 1;
			}
			...
			to.y -= 2;
			to_i = to.y * grid.w + to.x;
			if (to.y > 0 && grid.grid[to_i] != '#' && dist[to_i] - dist[from_i] >= 102) {
				count += 1;
			}
			...
			to.y += 2;
			to_i = to.y * grid.w + to.x;
			if (to.y < grid.h - 1 && grid.grid[to_i] != '#' && dist[to_i] - dist[from_i] >= 102) {
				count += 1;
			}
		}
	}
