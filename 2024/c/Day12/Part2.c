...
parameters_t region(grid_t grid, int x, int y, char el) {
    ...
    // outside top left
    if ((x == 0 || grid.grid[index - 1] != el) && (y == 0 || grid.grid[index - grid.w] != el)) {
        res.perm += 1;
    }
    // outside top right
    if ((x == grid.w - 1 || grid.grid[index + 1] != el) && (y == 0 || grid.grid[index - grid.w] != el)) {
        res.perm += 1;
    }
    // outside bottom left
    if ((x == 0 || grid.grid[index - 1] != el) && (y == grid.h - 1 || grid.grid[index + grid.w] != el)) {
        res.perm += 1;
    }
    // outside bottom right
    if ((x == grid.w - 1 || grid.grid[index + 1] != el) && (y == grid.h - 1 || grid.grid[index + grid.w] != el)) {
        res.perm += 1;
    }

    // inside top left
    if ((x > 0 && grid.grid[index - 1] == el) && (y > 0 && grid.grid[index - grid.w] == el) &&
        grid.grid[index - 1 - grid.w] != el) {
        res.perm += 1;
    }
    // inside top right
    if ((x < grid.w - 1 && grid.grid[index + 1] == el) && (y > 0 && grid.grid[index - grid.w] == el) &&
