...
static inline void grid_set(grid_t grid, size_t row, size_t col, char val) { grid.grid[row * grid.max_w + col] = val; }
...
int main(int argc, char **argv) {
	...
	do {
		...
		if (removed_one) {
			for (size_t row = 0; row < grid.h; ++row) {
				for (size_t col = 0; col < grid.w; ++col) {
					if (grid_get(grid, row, col) == 'x') {
						grid_set(grid, row, col, '.');
					}
				}
			}
		}
